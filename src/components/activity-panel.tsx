...
const actionConfig: Record<ActivityItem["action"], { icon: typeof Bell; label: string; color: string }> = {
  upload: { icon: Upload, label: "Uploaded", color: "text-emerald-500" },
  download: { icon: Download, label: "Downloaded", color: "text-sky-500" },
  rename: { icon: Pencil, label: "Renamed", color: "text-amber-500" },
  delete: { icon: Trash2, label: "Deleted", color: "text-destructive" },
  star: { icon: Star, label: "Starred", color: "text-yellow-500" },
  share: { icon: Share2, label: "Shared", color: "text-purple-500" },
  move: { icon: FolderInput, label: "Moved", color: "text-blue-500" },
  copy: { icon: Copy, label: "Copied", color: "text-teal-500" },
  create: { icon: FolderPlus, label: "Created", color: "text-emerald-500" },
};
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="icon" className="relative h-8 w-8" title="Activity log">
